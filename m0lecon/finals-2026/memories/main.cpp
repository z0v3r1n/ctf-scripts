...
struct Memory {
    ...
    Memory(int64_t ts, const char *desc) : timestamp(ts), description{} {
        if (desc) { strncpy(this->description, desc, sizeof(description) - 1); this->description[sizeof(description) - 1] = '\0'; }
    }
...
public:
    ...
    void add(const T &value) {
        ...
        if (!(data.empty() || value >= data.front())) data.sort(greater<T>());
    }
    ...
    void erase(const T &value) { data.remove_if([&value](const T &item) { return item == value; }); }
...
void read_exactly(char *b, size_t n) {
    for (size_t r = 0; r < n; cin.clear()) { cin.read(b + r, n - r); r += cin.gcount(); }
}

tag_t hash_tag(const char *s, size_t len) {
    tag_t total = 0, b;
    size_t i = 0, dim = sizeof(tag_t);
    for (; i + dim <= len; i += dim) { memcpy(&b, s + i, dim); total += b; }
    if (i < len) { b = 0; memcpy(&b, s + i, len - i); total += b; }
    return total;
}
...
int main() {
    ...
    while (cin.good()) {
        ...
        case 2:
            ...
            if (it != nullptr) it->tags.push_back(tag);
            break;
        ...
        case 3:
            ...
            tag = hash_tag(input, sizeof(input));
            it = memories.find(Memory(ts));
            if (it != nullptr) { len = it->tags.size(); if (len) it->tags[abs(idx) % len] = tag; }
            break;
